...
def predict_heart_disease(data):
    ...
    for modelname in modelnames:
        ...
        if hasattr(model, "feature_names_in_"):
            # Explicit mapping between app input and training dataset columns
            rename_map = {
                "age": "Age",
                "sex": "Sex",
            ...
            aligned_data = data.rename(columns=rename_map)

            # Ensure the same order as training
            aligned_data = aligned_data[model.feature_names_in_]
        else:
            aligned_data = data  # fallback
...
with tab3:
    ...
    fig.update_layout(yaxis=dict(range=[0, 100]))  # show accuracies from 0 to 100
